...
// BLoC
class ReviewBloc extends Bloc<DataEvent, DataState> {

  ReviewBloc() : super(InitialDataState());
  @override
  Stream<DataState> mapEventToState(DataEvent event) async* {
    final List<UserReview> _tempList = [] ;
    if (event is FetchDataEventForUserDetails) {
      yield LoadingDataState();

      try {
        final response = await http.get(
          Uri.parse(ApiUrls.getBaseUrl()+ApiUrls.comments),
          headers: {
        ...
        if (response.statusCode == 200) {
          ...
          {
            _tempList.add(UserReview.fromJSON(data[i]));
            print("---------------list----------$i");
          }

          yield LoadedDataStateForUserReview(reviewList: _tempList );
        } else {
